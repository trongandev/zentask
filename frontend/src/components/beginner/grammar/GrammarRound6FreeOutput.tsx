...
export function GrammarRound6FreeOutput({ topicId, isCorrect, setIsCorrect, data }: GrammarRound6FreeOutputProps) {
  ...
  const handleCheck = () => {
    ...
    setTimeout(() => {
      setIsChecking(false);
      // Giả lập logic chấm: Câu có ít nhất 3 chữ và dài hơn 10 ký tự
      const words = inputText.trim().split(/\s+/);
      if (words.length >= 3 && inputText.length > 10) {
        setIsCorrect(true);
        setFeedback("Câu của bạn rất tự nhiên và chính xác!");
      } else {
        setIsCorrect(false);
        setFeedback("Câu này có vẻ quá ngắn hoặc chưa đủ ý. Hãy viết một câu hoàn chỉnh hơn nhé!");
      }
    }, 1500);
  };
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right text-center flex flex-col items-center">
      ...
      <div className="w-full max-w-lg space-y-4">
        ...
        {feedback && <div className={cn("p-4 rounded-xl text-left font-medium", isCorrect ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800")}>{feedback}</div>}
