  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right">
      ...
      <div className="text-center mb-8">
        ...
        >
          {isLoading && loadingText === currentWord?.term ? <Loader2 className="w-6 h-6 animate-spin" /> : <Volume2 className="w-6 h-6" />}
        </Button>
      </div>
        ...
        {options.map((opt) => {
          ...
          return (
            <Button
              ...
              className={cn("p-4 rounded-2xl border-2 text-left font-bold text-lg transition-all duration-300 shadow-sm relative overflow-hidden", stateClass)}
            >
              {opt.meaning}
              {isCorrect !== null && opt.meaning === currentWord.meaning && (
