...
export function Round5FillBlank({ topicId, currentWord, allLessonWords, isCorrect, onCheckAnswer }: Round5FillBlankProps) {
  ...
  const sentenceWithBlank = useMemo(() => {
    ...
    const example = currentWord.example?.[0]?.en;
  ...
  const targetAudioText = currentWord?.example?.[0]?.en || currentWord?.term;
  return (
    <div className="space-y-6 animate-in slide-in-from-right text-center flex flex-col items-center">
      ...
      <div className="bg-white p-8 rounded-3xl border-2 border-slate-100 shadow-sm w-full max-w-lg mb-8">
        ...
        >
          {isLoading && loadingText === targetAudioText ? <Loader2 className="w-8 h-8 animate-spin" /> : <Volume2 className="w-8 h-8" />}
        </Button>
          ...
          <div className="text-2xl font-medium text-slate-700 leading-relaxed mb-4">
            ...
            <span
              className={cn(
                ...
                isCorrect === null ? "border-slate-300 text-transparent" : isCorrect === true ? "border-green-500 text-green-600" : "border-red-500 text-red-600",
              )}
            >
        ...
        {options.map((opt) => {
          ...
          return (
            <Button
              ...
              className={cn("p-4 rounded-2xl border-2 font-bold text-lg transition-all duration-300 shadow-sm relative overflow-hidden text-left", stateClass)}
            >
              {opt.term}
