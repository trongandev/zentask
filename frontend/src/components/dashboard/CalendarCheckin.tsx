...
export function CalendarCheckin() {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            setLoading(true);
            try {
                const res = await axiosInstance.get(`/api/user/calendar-stats?year=${year}&month=${month}`);
                if (res.status === 200) {
                    const data = res.data;
                    setStats(data);
                }
    ...
    const handleCheckIn = async () => {
        ...
        if (res) {
            ...
            setLoading(true);
            const resStats = await axiosInstance.get(`/api/user/calendar-stats?year=${year}&month=${month}`);
