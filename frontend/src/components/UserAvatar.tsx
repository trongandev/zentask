...
export function UserAvatar({ src, level, alt = "Avatar", className, avatarClassName, uid, disableLink = false }: UserAvatarProps) {
  ...
  const Content = (
    <>
      <img src={src} alt={alt} className={cn("w-[78%] h-[78%] rounded-full object-cover absolute", avatarClassName)} />
      <img src={getFrameSrc(level)} alt={`Level ${level} frame`} className="w-full h-full object-contain relative z-10 drop-shadow-sm" />
    </>
  );

  const containerClass = cn("relative inline-flex items-center justify-center flex-shrink-0", className);
  ...
  return (
    <Link to={`/profile/${uid}`} className={containerClass}>
      {Content}
    </Link>
  );
}
