...
export default function BotConfigPage() {
  ...
  const handleAddDefaultConfigs = async () => {
    try {
      ...
      toast.success("Đã thêm các cấu hình Rank còn thiếu!");
      fetchConfigs();
    } catch (err: any) {
  ...
  const handleSave = async (config: BotConfig) => {
    try {
      ...
      toast.success(`Đã lưu cấu hình Rank ${config.rankName}`);
      fetchConfigs();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={handleAddDefaultConfigs}
      ...
      {configs.length === 0 ? (
        <div className="bg-white rounded-2xl border p-12 text-center text-gray-500">
          Chưa có cấu hình Bot nào. Hãy ấn nút "Thêm Rank còn thiếu" để tạo mặc định.
        </div>
      ) : activeTab === "config" ? (
