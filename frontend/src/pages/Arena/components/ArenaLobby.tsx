                ...
                {(player?.rankId || player?.rankInfo) && (
                    <div className="flex items-center justify-center gap-1.5 bg-black/40 px-2 py-0.5 rounded-full border border-white/10">
                        ...
                        <p className={cn("font-bold text-[10px] md:text-xs text-center line-clamp-1", textColor)}>
                            {player?.rankId ? `${RANK_NAMES[player.rankId] || "Bạc"} ${["I", "II", "III"][Math.max(0, Math.min(2, (player.tier || 3) - 1))]}` : player?.rankInfo}
                        </p>
                    </div>
                )}
            </div>
            {player?.isBot && (
                <p className="mt-1 rounded-full bg-yellow-500/15 px-3 py-1 text-[11px] font-extrabold text-yellow-200">Bot {player.botAccuracyLabel ? `• ${player.botAccuracyLabel}` : ""}</p>
            )}
        </div>
    );
}
...
function OpponentPicker({
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            setIsSearching(true);
            try {
                const res = await axiosInstance.get(`/api/friends/search?q=${encodeURIComponent(searchQuery)}`);
                const data = res.data;
    ...
    const handleSendFriendRequest = async (uid: string, e: React.MouseEvent) => {
        e.stopPropagation();
        try {
            const res = await axiosInstance.post(`/api/friends/request`, { userId: uid });
            const data = res.data;
            if (res.status === 200) {
                                    ...
                                    <button
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="text-sm font-bold text-white truncate flex items-center gap-2">
                                                ...
                                                {u.friendStatus === "sent" && searchQuery.length >= 2 && (
                                                    <span className="bg-white/10 text-white/50 px-2 py-0.5 rounded text-[10px] uppercase font-bold">Đã gửi</span>
                                                )}
                                            </div>
...
function FriendRequestsWidget() {
    ...
    const fetchRequests = async () => {
        try {
            ...
            const data = res.data;
            if (res.status === 200 && data.incoming) {
    ...
    const handleRespond = async (id: string, action: "accept" | "decline") => {
        try {
            const res = await axiosInstance.post(`/api/friends/request/${id}/respond`, { action });
            if (res.status === 200) {
            ...
            <div className="flex flex-col items-center z-10 w-full max-w-4xl px-4 animate-in fade-in zoom-in duration-500">
                            ...
                            {prepCountdown > 0 ? (
                                ...
                                <div className="flex flex-col items-center gap-3 relative z-20">
                                    ...
                                    {allOthersReady && !isReady && isHost && (
                                        <span className="text-emerald-400 text-sm font-bold animate-pulse mt-2 bg-black/60 px-3 py-1 rounded-full border border-emerald-500/30">
                        ...
                        {isFound && opponent ? (
                            <PlayerSlot
                                player={{
                                    ...
                                    rankInfo: opponent.rankInfo,
                                    isBot: opponent.isBot,
                                    botAccuracyLabel: opponent.botAccuracyLabel,
                                }}
                                side="right"
                                canKick={isHost}
                                ...
                                onAddFriend={
                                    opponent.uid !== user?.uid &&
                                    !opponent.isBot &&
                                    friendStatuses[opponent.uid] !== "friend" &&
                                    !friends.some((f) => f.uid === opponent.uid || f.friendId === opponent.uid)
                ...
                <div className="mt-8 flex gap-4 items-center">
                    ...
                    {isSearching ? (
                        ...
                        <button
                            onClick={isFound ? onCancelSearch : onBack}
                                    ...
                                    lobbyMessages.map((msg, i) => {
                                        ...
                                        return (
                                            <div key={i} className={cn("flex flex-col max-w-[85%]", isMe ? "self-end items-end" : "self-start items-start")}>
                                                ...
                                                <div
                                                    className={cn("px-3 py-2 rounded-2xl text-sm break-words", isMe ? "bg-blue-600 text-white rounded-br-sm" : "bg-white/10 text-white rounded-bl-sm")}>
                                                    {msg.text}
                                                </div>
                                            </div>
                                        );
