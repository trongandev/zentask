...
export function Arena() {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get(`/api/arena/friends`, { withCredentials: true })
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get(`/api/arena/history`, { withCredentials: true })
      .then((res) => (res.data.success ? res.data : []))
  ...
  const fetchLobbyFriends = useCallback(() => {
    ...
    axiosInstance
      .get(`/api/friends/online`, { withCredentials: true })
      .then((res) => (res.data.success ? res.data : []))
  ...
  useEffect(() => {
    ...
    const onMatchFound = (data: any) => {
      ...
      axiosInstance.post(`/api/arena/stats/matchmaking`, { uid: user.uid, durationMs, rankId: user.rankId, tier: user.tier }, { withCredentials: true }).catch(console.error);
    ...
    socket.on("arena_end_game", async (data: any) => {
      ...
      if (finalMode === "tournament") {
        try {
          ...
          if (code) {
            const res = await axiosInstance.post(`/api/arena/tournaments/${code}/complete`);
            const xpData = await res.data;
            if (res.status === 200 && xpData.xpResult) updateUser({ xp: xpData.xpResult.xp, level: xpData.xpResult.level });
            if (res.status === 200) toastService.success(xpData.awardedXp ? "+10XP giải đấu" : "Giải đấu không tính rank");
          }
        } catch (err) {
  ...
  const createTournamentRoom = async () => {
    const res = await axiosInstance.post(`/api/arena/tournaments`, {
      title: tournamentTitle,
      inviteUserIds: selectedInviteIds,
    });
    const data = await res.data;
    if (res.status !== 200) return toastService.error(data.error || "Không tạo được giải đấu");
  ...
  const joinTournamentByCode = async () => {
    ...
    const res = await axiosInstance.post(`/api/arena/tournaments/${tournamentCode.trim()}/join`);
    const data = await res.data;
    if (res.status !== 200) return toastService.error(data.error || "Không vào được phòng");
    setTournamentRoom(data);
    joinTournamentLobby(data);
