...
export default function SkillPracticeRoom() {
    ...
    const fetchDaily = async () => {
        const res = await axiosInstance.get(`/api/skill-practice/daily`);
        if (res.status === 200) setDaily(res.data);
    };
    ...
    const loadExercise = async () => {
        ...
        try {
            const res = await axiosInstance.get(`/api/skill-practice/random?mode=${mode}&t=${Date.now()}`);
            if (res.status === 200) {
                setExercise({ ...res.data, _renderKey: `${mode}_${res.data.id || res.data.variantId || "exercise"}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}` });
    ...
    const submit = async (isCorrect: boolean, responseMs = Date.now() - startedAt, extra: any = {}) => {
        ...
        try {
            const res = await axiosInstance.post(`/api/skill-practice/submit`, { mode, isCorrect, responseMs, exerciseId: exercise?.id });
            if (res.status === 200) {
                setResult({ isCorrect, responseMs, correctAnswer: correctAnswerOf(exercise), ...extra, ...res.data });
                setDaily({ completedModes: res.data.completedModes, bonusClaimed: res.data.bonusClaimed });
                if (res.data.xpResult) updateUser({ xp: res.data.xpResult.xp, level: res.data.xpResult.level });
            }
            if (res.data.awardedXp > 0) toastService.success(`+${res.data.awardedXp} XP`);
