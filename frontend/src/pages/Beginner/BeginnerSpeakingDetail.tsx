...
import { ArrowLeft, Play, Mic, Square, CheckCircle2, RefreshCw } from "lucide-react";
...
const SPEAKING_DATA: Record<string, { title: string; sentences: string[] }> = {
  ...
  "morning-routine": {
    ...
    sentences: [
      "I usually wake up at seven AM.",
      "Then I brush my teeth and wash my face.",
      "I like to have toast and eggs for breakfast.",
      "I leave for work around half past eight."
    ]
  },
  "meeting-friend": {
  ...
  "reading-books": {
    ...
    sentences: [
      "I really enjoy reading books on weekends.",
...
export function BeginnerSpeakingDetail() {
  ...
  const data = id ? SPEAKING_DATA[id] : null;

  const currentSentence = data?.sentences[currentIndex] || "";

  // Hook for playing reference audio
  const { playAudio: playTTS, isPlaying: isTTSPlaying } = useTTSAudio();
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Button onClick={() => navigate("/beginner/speaking")} className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-slate-900 bg-transparent shadow-none hover:bg-slate-100 border-none">
      ...
      <div className="bg-white rounded-3xl p-8 border-2 border-green-100 shadow-xl w-full text-center">
        ...
        <div className="text-slate-400 text-sm font-medium mb-8">Câu {currentIndex + 1} / {data.sentences.length}</div>
        ...
        <div className="flex items-center justify-center gap-6">
          <Button 
            disabled={isTTSPlaying || status === "recording" || status === "checking"}
            onClick={() => playTTS(currentSentence, "en-US-JennyNeural")}
        ...
        {status === "checking" && (
          <p className="text-sm font-bold text-blue-500 mt-6 animate-pulse">
