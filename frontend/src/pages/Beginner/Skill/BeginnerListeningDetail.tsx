          ...
          {isFinished ? (
            <div className="bg-slate-50 rounded-3xl p-8 border border-slate-200 text-center animate-in zoom-in-95 duration-500">
              ...
              <p className="text-xl text-slate-600 mb-8">
                Bạn đã trả lời đúng <span className="font-black text-indigo-600">{correctCount}</span> / {data.questions.length} câu hỏi.
              </p>
              <Button onClick={() => navigate("/beginner/listening")} className="px-10 py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg text-lg">
                Quay lại danh sách
              </Button>
            </div>
          ) : (
            currentQuestion && (
              <div className="bg-slate-50 rounded-2xl p-6 md:p-8 border border-slate-200 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  {currentQuestion.options.map((opt: string) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {!isAnswerChecked && !isSelected && <div className="w-5 h-5 rounded-full border-2 border-slate-300 group-hover:border-indigo-300 flex-shrink-0" />}
                        {!isAnswerChecked && isSelected && <div className="w-5 h-5 rounded-full border-[6px] border-indigo-600 flex-shrink-0" />}
                      </div>
                    );
                  })}
                </div>
                ...
                {isAnswerChecked && currentQuestion.explanation && (
                  <div className="mb-8 p-5 bg-indigo-50 text-indigo-900 rounded-xl border border-indigo-100 animate-in fade-in zoom-in-95">
                    <span className="font-black text-indigo-700 block mb-1">Giải thích: </span>
                ...
                {!isAnswerChecked ? (
                  <Button
                    ...
                    className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg text-lg disabled:opacity-50 transition-all"
                  >
                    Kiểm tra đáp án
                  </Button>
                ) : (
