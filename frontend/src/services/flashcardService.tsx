...
interface FlashcardState {
  ...
  generateAIList: (words: string[], setId: string) => Promise<any[] | null>;
...
export const useFlashcardStore = create<FlashcardState>((set, get) => ({
  ...
  generateAIList: async (words, setId) => {
    set({ loading: true });
    try {
      const res = await axiosInstance.post(`/api/flashcard/generate-ai-list`, { words, setId });
      toastService.success(`Đã tạo thành công bộ ${words.length} từ vựng!`);
      return await res.data;
    } catch (err: any) {
      return null;
    } finally {
