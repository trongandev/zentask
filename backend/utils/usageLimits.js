...
import { DailyUsage } from "../src/models/Schemas.js";
...
export async function consumeDailyLimit({ uid, key, amount = 1, limit, message }) {
  ...
  const safeAmount = Math.max(1, Math.floor(Number(amount) || 1));
  const date = getTodayKey();
  const updated = await DailyUsage.findOneAndUpdate({ uid, date, key }, { $inc: { count: safeAmount }, $setOnInsert: { uid, date, key } }, { upsert: true, new: true }).lean();
