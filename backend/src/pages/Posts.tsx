...
export function Posts() {
  ...
  const filteredPosts = activeCategory === "all" 
    ? POSTS 
    : POSTS.filter(post => post.category === CATEGORIES.find(c => c.id === activeCategory)?.label);

  const getCategoryColor = (category: string) => {
    switch (category) {
      case "Kinh nghiệm học": return "text-blue-600 bg-blue-50";
      case "Kinh nghiệm thi": return "text-orange-600 bg-orange-50";
      case "Tin tức Zentask": return "text-green-600 bg-green-50";
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case "Kinh nghiệm học": return <BookOpen className="w-3 h-3" />;
      case "Kinh nghiệm thi": return <Star className="w-3 h-3" />;
      case "Tin tức Zentask": return <Zap className="w-3 h-3" />;
      default: return null;
    }
  };
          ...
          >
            <div className="relative h-48 overflow-hidden">
              ...
              <div className="absolute top-4 left-4">
                <span className={cn("px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 shadow-sm backdrop-blur-md", 
                  post.category === "Kinh nghiệm học" ? "bg-blue-500/90 text-white" : 
                  post.category === "Kinh nghiệm thi" ? "bg-orange-500/90 text-white" : 
