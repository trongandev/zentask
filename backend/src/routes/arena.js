...
import { ArenaMatchmakingStat, ArenaTournamentRoom, Friendship, UserActivity } from "../models/Schemas.js";
...
router.get(
  "/history",
  asyncHandler(async (req, res) => {
    const activities = await UserActivity.find({ uid: req.user.uid, type: "arena" })
      .sort({ createdAt: -1 })
      .limit(20)
      .lean();
    res.json(activities);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const rooms = await ArenaTournamentRoom.find({
      $or: [{ hostId: req.user.uid }, { participantIds: req.user.uid }, { invitedUserIds: req.user.uid }],
    })
      .sort({ createdAt: -1 })
      .limit(30)
      .lean();
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const participants = await User.find({ _id: { $in: room.participantIds || [] } })
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const friendIds = docs
      .flatMap((doc) => doc.users || [])
      .map(String)
