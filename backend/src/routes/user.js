...
export const addXpToUser = async (uid, xpToAdd) => {
  ...
  await LeaderboardWeekly.findOneAndUpdate(
    { uid, period: weekString },
    { $inc: { xp: xpToAdd } },
    { upsert: true, new: true }
  );

  await LeaderboardMonthly.findOneAndUpdate(
    { uid, period: monthString },
    { $inc: { xp: xpToAdd } },
    { upsert: true, new: true }
  );
...
// Record study time
router.post("/study-time", asyncHandler(async (req, res) => {
  ...
  const stat = await UserDailyStat.findOneAndUpdate(
    { userId: uid, date: today },
...
// Gain XP endpoint
router.post("/gain-xp", asyncHandler(async (req, res) => {
...
router.post("/onboarding/reset", asyncHandler(async (req, res) => {
...
router.get("/daily-tasks/progress", asyncHandler(async (req, res) => {
  const today = getLocalDateString();
...
router.get("/beginner-progress", asyncHandler(async (req, res) => {
...
router.post("/beginner-progress", asyncHandler(async (req, res) => {
...
router.post("/beginner-topic-complete", asyncHandler(async (req, res) => {
...
router.post("/follow/:uid", asyncHandler(async (req, res) => {
...
// Check if following
router.get("/follow/:uid", asyncHandler(async (req, res) => {
  const { uid: targetUid } = req.params;
