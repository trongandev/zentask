...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await consumeDailyLimit({
      ...
      key: "tenses_generate",
      amount: 1,
      message: "Bạn đã tạo đủ 2 bài tenses hôm nay. Nâng VIP để tạo không giới hạn.",
    });
      ...
      if (recentMistakes.length > 0) {
        recentMistakesStr =
          ...
          recentMistakes
            .map(
              (m) =>
                `- Câu hỏi (${m.type}): "${m.question}". Thời gian trả lời: ${m.timeSpent}s.\n  Đáp án học viên đã chọn: ${typeof m.userAnswer === "object" ? JSON.stringify(m.userAnswer) : m.userAnswer}`,
            )
            .join("\n");
      }
    }
