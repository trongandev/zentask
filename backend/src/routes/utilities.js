...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const items = await CalculatorHistory.find({ uid: req.user.uid }).sort({ createdAt: -1 }).limit(60).lean();
...
router.get(
  "/translation-history",
  asyncHandler(async (req, res) => {
    const items = await TranslationHistory.find({ uid: req.user.uid }).sort({ createdAt: -1 }).limit(80).lean();
    res.json({ items: items.map(serializeDoc) });
  }),
);
...
router.get(
  "/study-methods",
  asyncHandler(async (req, res) => {
    const items = await StudyMethod.find({ uid: req.user.uid }).sort({ createdAt: -1 }).limit(50).lean();
    res.json({ items: items.map(serializeDoc) });
  }),
);
